...
class Pointnet2Backbone(nn.Module):
    ...
    def __init__(self, input_feature_dim=0):
        ...
        # --------- ISL 只放在 SA1 后（点数 2048） ---------
        # sa1 输出通道是 128，所以 in_channel 写成 2 * 128（ISL 内部用 in_channel // 2）
        self.isl_sa1 = ISL(
            in_channel=2 * 128,
    ...
    def forward(self, data_dict):
        ...
        # --------- 在 SA1 输出上做 ISL（N=2048，非常安全） ---------
        if self.isl_sa1 is not None:
            # knn 用几何坐标建图
            xyz_knn = xyz.transpose(1, 2).contiguous()      # (B, 3, 2048)
            idx = knn(xyz_knn, k=20)                        # 直接固定 k=20 即可
            features = self.isl_sa1(features, idx)          # (B, 128, 2048)
            data_dict['sa1_features'] = features
        ...
        features = self.fp2(
            data_dict['sa2_xyz'], data_dict['sa3_xyz'], data_dict['sa2_features'], features)
