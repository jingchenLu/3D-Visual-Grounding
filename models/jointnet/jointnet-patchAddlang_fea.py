...
from models.patch_fusion.patch_fusion_module import PatchLanguageFusion
...
class JointNet(nn.Module):
    def __init__(self, num_class, num_heading_bin, num_size_cluster, mean_size_arr, vocabulary, embeddings=None,
        ...
        if not no_reference:
            ...
            ###############################################
            #     SA2 Patch-to-Language Fusion Module     #
            ###############################################
            self.patch_fusion = PatchLanguageFusion(
                pc_feat_dim=256,                   # sa2_features 通道数=256
    ...
    def forward(self, data_dict, use_tf=True, is_eval=False):
        ...
        # --------- HOUGH VOTING ---------
        xyz = data_dict["fp2_xyz"]   #(B, N, 3) (8, 1024, 3)
        features = data_dict["fp2_features"] #(B, C, N) (8, 256, 1024)
        data_dict["seed_inds"] = data_dict["fp2_inds"] #(B, N)
        data_dict["seed_xyz"] = xyz
        data_dict["seed_features"] = features

        xyz, features = self.vgen(xyz, features)
        features_norm = torch.norm(features, p=2, dim=1) # (B, N)
        features = features.div(features_norm.unsqueeze(1)) #(B, C, N)
        ...
        # bbox_features:  (8,256,128) dist_weights : ([8, 4, 256, 256])
        if not self.no_reference:
            ...
            #                                     #
            #######################################
            data_dict = self.lang(data_dict) # lang_feat:([64, 50, 128]) lang_emb:([64, 128]) lang_scores [64, 18]

            # ---------- Patch-level Text-Cloud Fusion ----------
            data_dict = self.patch_fusion(data_dict)
