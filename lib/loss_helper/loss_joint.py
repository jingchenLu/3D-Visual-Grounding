...
from .loss_grounding import compute_reference_loss, compute_lang_classification_loss, compute_diou_loss, compute_attr_loss, compute_vote_weight_loss
from .loss_pretrain import compute_constrastive_loss
from .loss_answering import compute_answer_classification_loss
...
def get_joint_loss(args, data_dict, device, config, weights, pad_token_id,
                   detection=True, caption=True, reference=True, use_lang_classifier=True,
                   orientation=False, distance=False, num_bins=CONF.TRAIN.NUM_BINS, num_ground_epoch=50, tokenizer=None):
    """ Loss functions
    ...
    if reference:
        # Reference loss
        data_dict, ref_loss, _, cluster_labels = compute_diou_loss(
            data_dict, config, use_reg_head=args.use_reg_head, use_kl_loss=args.use_kl_loss, debug=args.debug)
    ...
    if reference and args.use_attr_loss:
        data_dict["attr_loss"] = compute_attr_loss(data_dict)

    if reference and args.use_vote_weight:
        data_dict["vote_weight_loss"] = compute_vote_weight_loss(data_dict)

    if args.use_answer:
        data_dict["answer_loss"] = compute_answer_classification_loss(
            data_dict)

    if caption:
        cap_loss, cap_acc = compute_cap_loss(
            data_dict, config, weights, pad_token_id=pad_token_id, tokenizer=tokenizer)
    ...
    if detection:
        ...
        loss = data_dict["vote_loss"] + 0.1 * \
            data_dict["objectness_loss"] + data_dict["box_loss"]
        loss *= 10  # amplify
        ...
        if args.use_kl_loss:
            loss += 0.3*data_dict["kl_loss"]
        if use_lang_classifier:
            loss += 0.3*data_dict["lang_loss"]
        if args.use_attr_loss:
            loss += 0.3*data_dict["attr_loss"]
        if args.use_vote_weight:
            loss += 0.3*data_dict["vote_weight_loss"]
    else:
        raise NotImplementedError()
        loss = 0.
        if caption and data_dict["epoch"] < num_ground_epoch:
    ...
    if args.use_mlm:
        # if data_dict["epoch"] >= 50:
        loss +=10*data_dict["mlm_loss"]
    else:
        data_dict["mlm_loss"] = torch.zeros(1)

    if args.use_answer:
        loss += data_dict["answer_loss"]
