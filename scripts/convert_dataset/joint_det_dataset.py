...
SCANNET_V2_TSV = os.path.join("data/meta_data/scannetv2-labels.combined.tsv")
...
class Joint3DDataset(Dataset):
    ...
    def _get_raw2label(self):
        # mapping
        scannet_labels = DC18.type2class.keys()
        ...
        for i in range(len(lines)):
            ...
            if nyu40_name not in label_classes_set:
                raw2label[raw_name] = scannet2label['other furniture']
            else:
    ...
    def __getitem__(self, index):
        ...
        if anno['dataset'] == 'scannet':
            # self.random_utt = self.joint_det and np.random.random() > 0.1
            self.random_utt = self.joint_det
        ...
        ret_dict.update({
            ...
            "utterances": (
                ' '.join(anno['utterance'].replace(',', ' ,').split())  #+ ' . not mentioned'
            ),
