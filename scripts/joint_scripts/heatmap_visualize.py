...
def dump_results(args, scanrefer, data, config):
    ...
    for i in range(batch_size):
        ...
        # Todo Get The Visualization of the BBox
        pred_corner = data['pred_bbox_corner'].detach().cpu()
        relation = torch.cat(data['relation'], dim=1)[i].permute(1, 2, 0)
        mask = nms_mask[i].bool()
        bbox_corner = pred_corner[i, mask].numpy()
        relation = relation[mask].permute(2, 1, 0)
        ...
        save_bbox_heatmap(bbox_corner, relation,
                            save_base=scene_dump_dir,
                            save_name='pred_heatmap_{}_{}_{}_{:.5f}_{:.5f}.obj'.format(object_id, object_name, ann_id, pred_ref_scores_softmax[i, pred_ref_idx], iou),
                            kth_input=pred_ref_idx)
...
def visualize(args):
    ...
    for data in tqdm(dataloader):
        ...
        # nms mask
        parse_predictions(data, POST_DICT)
        nms_masks = torch.LongTensor(data["pred_mask"]).cuda()
        # objectness mask
        obj_masks = torch.argmax(data["objectness_scores"], 2).long()

        # final mask
        nms_masks = nms_masks * obj_masks
        data['nms_mask'] = nms_masks
        # visualize
