...
# -----------------------------------------------------------
# Convert from box parameters
# Notice: This is for Camera view (camera X,Y,Z = depth X,-Z,Y)
# -----------------------------------------------------------
def roty(t):
...
# -----------------------------------------------------------
# Notice: This is for Depth View (camera X,Y,Z = depth X,-Z,Y)
# -----------------------------------------------------------
...
def box3d_diou_batch_tensor(center1, size1, center2,size2):
    center_x1 = center1[:,0]
    center_y1 = center1[:,1]
    center_z1 = center1[:,2]
    center_x2 = center2[:,0]
    ...
    inter_xA = torch.max(center_x1-l1/2,center_x2-l2/2)
    inter_xB = torch.min(center_x1+l1/2,center_x2+l2/2)
    inter_yA = torch.max(center_y1-w1/2,center_y2-w2/2)
    inter_yB = torch.min(center_y1+w1/2,center_y2+w2/2)
    inter_zA = torch.max(center_z1-h1/2,center_z2-h2/2)
    inter_zB = torch.min(center_z1+h1/2,center_z2+h2/2)
    inter_area = torch.clamp((inter_xB-inter_xA),min=0)*torch.clamp((inter_yB-inter_yA),min=0)*torch.clamp((inter_zB-inter_zA),min=0)

    union = area1+area2 - inter_area
    iou = inter_area/union

    # cal dist between
    inter_diag = (center_x1-center_x2)**2 + (center_y1-center_y2)**2 + (center_z1-center_z2)**2

    outer_xA = torch.min(center_x1-l1/2,center_x2-l2/2)
    outer_xB = torch.max(center_x1+l1/2,center_x2+l2/2)
    outer_yA = torch.min(center_y1-w1/2,center_y2-w2/2)
    outer_yB = torch.max(center_y1+w1/2,center_y2+w2/2)
    outer_zA = torch.min(center_z1-h1/2,center_z2-h2/2)
    outer_zB = torch.max(center_z1+h1/2,center_z2+h2/2)
    outer_diag = torch.clamp((outer_xB-outer_xA),min=0)**2 + torch.clamp((outer_yB-outer_yA),min=0)**2 + torch.clamp((outer_zB-outer_zA), min=0)**2
